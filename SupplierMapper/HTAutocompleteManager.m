             ...
             ignoreCase:(BOOL)ignoreCase
{
    ...
    if (textField.autocompleteType == HTAutoCompleteTypePlant)
    {
        DataService *dataService = [DataService defaultService];
        NSArray *autocompleteArray = dataService.plantSearchList;
        ...
        if (autocompleteArray != nil)
        {
            ...
            for (NSString *searchKey in autocompleteArray)
            {
                NSString *stringToCompare;
                if (ignoreCase)
                {
                    stringToCompare = [searchKey lowercaseString];
                }
                else
                {
                    stringToCompare = searchKey;
                }
                
                if ([stringToCompare hasPrefix:stringToLookFor])
                {
                    // get the actual object for this key
                    PlantMaster *plant = nil;
                    
                    for (PlantMaster *item in [dataService.plantMasterItems allValues] )
                    {
                        if ([[item.SupplierID lowercaseString] isEqualToString:stringToCompare] || [[item.DescriptionAndCode lowercaseString] isEqualToString:stringToCompare])
                        {
                            if ([dataService.selectedPlantMasterItems objectForKey:item.SupplierID]==nil)
                            {
                                plant = item;
                                break;
                            }
                    ...
                    // Ignore any plant already in the selectedPlantItems dictionary
                    if (plant != nil)
                    {
                        return [searchKey stringByReplacingCharactersInRange:[stringToCompare rangeOfString:stringToLookFor] withString:@""];
                    }
                }
    ...
    else if (textField.autocompleteType == HTAutocompleteTypeProductCode)
    {
        DataService *dataService = [DataService defaultService];
        NSArray *autocompleteArray = dataService.pccSearchList;
        ...
        if (autocompleteArray != nil)
        {
            ...
            for (NSString *searchKey in autocompleteArray)
            {
                ...
                {
                    // get the actual object for this key
                    ProductCodeMaster *objSelected = nil;
                    
                    for (ProductCodeMaster *item in dataService.ProductCodeMasterItems)
                    {
                        if ([[item.ProductCode lowercaseString] isEqualToString:stringToCompare] || [[item.Description lowercaseString] isEqualToString:stringToCompare])
                        {
                            if ([dataService.selectedProductCodeMasterItems objectForKey:item.ProductCode]==nil)
                            {
                                objSelected = item;
                                break;
                            }
    ...
    if (textField.autocompleteType == HTautocompleteTypeSupplier)
    {
        DataService *dataService = [DataService defaultService];
        NSArray *autocompleteArray = dataService.supplierSearchList;
        ...
        if (autocompleteArray != nil)
        {
            ...
            for (NSString *searchKey in autocompleteArray)
            {
                ...
                {
                    // get the actual object for this key
                    Supplier *itemSelected = nil;
                    
                    for (Supplier *item in [dataService.supplierItems allValues] )
                    {
                        if ([[item.SupplierID lowercaseString] isEqualToString:stringToCompare] || [[item.DescriptionAndCode lowercaseString] isEqualToString:stringToCompare])
                        {
                            if ([dataService.selectedSupplierItems objectForKey:item.SupplierID]==nil)
                            {
                                itemSelected = item;
                                break;
                            }
    ...
    else if (textField.autocompleteType == HTAutocompleteTypeEmail)
    {
